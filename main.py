...
# Custom Hex Scheme matching Chunker's deep gray backgrounds and branding green
ctk.set_default_color_theme("green") 

# Target Version Matrix Definitions
...
class AmethystEngineApp:
    def __init__(self):
        ...
        # Load custom Minecraftia font safely (Fall back to Consolas if missing)
        self.font_title = ("Minecraftia", 24) if "Minecraftia" in ctk.FontManager.get_font_names() else ("Consolas", 24, "bold")
    ...
    def create_upload_screen(self):
        ...
        self.drop_lbl = ctk.CTkLabel(
            ...
            font=self.font_body,
            text_color="#A855F7", # Brand Amethyst
            justify="center",
            cursor="hand2"
        )
    ...
    def create_version_screen(self):
        ...
        grid_box = ctk.CTkFrame(self.version_frame, fg_color="#2D2D2D", padding=20)
        grid_box.pack(fill="both", expand=True)

        # Java Framework Column
        java_lbl = ctk.CTkLabel(grid_box, text="MINECRAFT: JAVA EDITION", font=self.font_body, text_color="#60A5FA")
        java_lbl.grid(row=0, column=0, padx=20, pady=10, sticky="n")
        
        self.java_menu = ctk.CTkOptionMenu(grid_box, values=JAVA_VERSIONS, font=self.font_body, width=220)
        self.java_menu.grid(row=1, column=0, padx=20, pady=5)
        ...
        btn_java.grid(row=2, column=0, padx=20, pady=15)

        # Bedrock Framework Column
        bedrock_lbl = ctk.CTkLabel(grid_box, text="MINECRAFT: BEDROCK", font=self.font_body, text_color="#F59E0B")
        bedrock_lbl.grid(row=0, column=1, padx=20, pady=10, sticky="n")
        
        self.bedrock_menu = ctk.CTkOptionMenu(grid_box, values=BEDROCK_VERSIONS, font=self.font_body, width=220)
        self.bedrock_menu.grid(row=1, column=1, padx=20, pady=5)
        ...
        btn_bedrock.grid(row=2, column=1, padx=20, pady=15)
    ...
    def browse_file(self, event=None):
        ...
        path = filedialog.askopenfilename(filetypes=[("Minecraft Compressed Archive", "*.zip;*.mcworld")])
    ...
    def conversion_worker(self):
        ...
        try:
            bytes_copied = 0
            buffer_size = 1024 * 1024 * 16 # High performance 16MB stream buffer
            ...
            with open(self.current_file, 'rb') as fsrc, open(output_path, 'wb') as fdst:
                while True:
                    ...
                    # Cycle console updates natively based on performance tracking
